...
const AdminNavbar = () => {
  ...
  return (
    <nav
      className={clsx(
        "w-full sticky top-0 bg-emerald-200 p-4",
    ...
    >
      <div className="container mx-auto flex justify-between items-center">
        ...
        <ul
          className={clsx(
            "hidden lg:flex lg:space-x-14 justify-center flex-grow",
            isMenuOpen && "lg:flex"
          )}
        >
          {navLinks.map((link) => (
            <li key={link.name}>
              <Link
                href={link.url}
                className="text-[18px] text-gray-700 font-bold hover:text-gray-500"
              >
                {link.name}
              </Link>
            </li>
